...
// Teleport cells
__kernel void move_to_opposite_border(__global float4* centers,
                            ...
                            __global const int* isDeleted)
{
  ...
  for (int row = max(0, sq_row-1); row < min((int)(sq_row+2), grid_y_range); row++) {
  	  ...
  	  // Is this is the earliest we can use the continue statement?
    ...
    for (int col = max(0, sq_col-1); col < min((int)(sq_col+2), grid_x_range); col++) {
      ...
      for (int n = sq_inds[sq]; n < (sq < n_sqs-1 ? sq_inds[sq+1] : n_cells); n++) {
    	...
    	int status_j = isDeleted[j];
  		if (status_j==1) continue; // we can't collide with deleted cells
...
__kernel void find_contacts_periodic(const int max_cells,
									 ...
									 __global float* reldists,
									 __global float* stiff,
									 __global int* sq_neighbours,
									 __global float4* offset_vecs,
									 __global int* test_offset_inds,
									 __global float4* test_offset_vecs,
									 __global float4* test_sep_vecs)
{
// our id
int i = get_global_id(0);

// debug counter for wraparound contacts
int wraparound_contact_counter = 0;
...
// define the periodic moore neighbourhood of this square [later: stop doing this on the fly]
sq_neighbours[6*i+0] = max(sq_col-1,0) - min(sq_col-1,0)*(grid_x_range-1);
sq_neighbours[6*i+1] = sq_col;
sq_neighbours[6*i+2] = (sq_col+1) % grid_x_range; 
sq_neighbours[6*i+3] = max(sq_row-1,0) - min(sq_row-1,0)*(grid_y_range-1);
sq_neighbours[6*i+4] = sq_row;
sq_neighbours[6*i+5] = (sq_row+1) % grid_y_range;

// loop through the cell's periodic Moore neighbourhood
for (int v_index_y = 0; v_index_y < 3; v_index_y++) {
  
  // what row are we on?
  int row = sq_neighbours[6*i+v_index_y+3];
  
	  for (int v_index_x = 0; v_index_x < 3; v_index_x++) {
	  
	  // what column are we on?
	  int col = sq_neighbours[6*i+v_index_x+0];
		
	  // what (neighbouring) square is this?
	  int sq = row*grid_x_range + col;
	  
	  // compare the columns, rows of the focal and neighbour squares
	  int delta_x = sq_col - col; // focal col - neighbour col
	  int delta_y = sq_row - row; // focal row - neighbour row
	  
	  // is neighbour next to focal square, or is it on the opposite border?
	  int offset_index_x = 0;
	  int offset_index_y = 0;
	  if (abs(delta_x) > 1)	offset_index_x = delta_x / (int) abs(delta_x); // doesn't seem to be a sign() function for ints
	  if (abs(delta_y) > 1)	offset_index_y = delta_y / (int) abs(delta_y);
	  
	  // index of offset vector for this neighbour
	  int offset_ind = offset_index_x + 3*offset_index_y + 4;
	  
	  // store the offset inds for inspection
	  test_offset_inds[9*i + v_index_y*3 + v_index_x] = offset_ind;
      ...
      for (int n = sq_inds[sq]; n < (sq < n_sqs-1 ? sq_inds[sq+1] : n_cells); n++) {
		...
		// we can't collide with ourself, only find low -> hi contacts
        if (j<=i) continue; 

        // Look for any existing contacts, count them, and store idx's
        int n_existing_cts=0;		// counter
        int existing_cts_idx[2];	// empty array
        for (int m=i*max_contacts; m<i*max_contacts+n_cts[i]; m++)
        {
          if (tos[m]==j)
          {
        ...
        // Are we within possible touching distance, or is there an existing contact to update?
		float offset_sep = length(centers[i] - centers[j] - offset);
        if (offset_sep > 0.5*(lens[i]+lens[j])+rads[i]+rads[j]+MARGIN && n_existing_cts==0)
        {
          // if not...
          continue;
        }
		...
		test_offset_vecs[9*i + v_index_y*3 + v_index_x] = offset;
		...
		//
      } // [END] this neighbour cell
    } // [END] this neighbour square (x-search)
  } // [END] this neighbour square (y-search)
  ...
  for (int u = k; u < max_contacts; u++) {
    tos[i*max_contacts+u] = 0;
  } // [END] wipe unused contacts
  
}


__kernel void find_contacts_periodic2(const int max_cells,
									 const int n_cells,
									 const int n_sqs,
									 const int max_contacts,
									 __global const float4* centers,
...
__kernel void collect_tos_periodic2(const int max_cells,
								   const int n_cells,
								   const int n_sqs,
								   const int max_contacts,
								   __global const int* sqs,
								   ...
								   __global const int* sq_neighbour_inds)
{
  ...
  // loop through the focal cell's 9 neighbours
  for (int neigh_index = 0; neigh_index < 9; neigh_index++) {
									...
									__global float4* offset_vecs)
{
  ...
  // calculate which offset to use when computing r_b for neighbour cell b
  int grid_x_range = grid_x_max-grid_x_min;
  int grid_y_range = grid_y_max-grid_y_min;
  
  // which row, column is focal cell "a" in?
  int sq_row_a = sqs[a] / grid_x_range; 
  int sq_col_a = sqs[a] % grid_x_range;
  
  // which row, column is neighbour cell "b" in?
  int sq_row_b = sqs[b] / grid_x_range; 
  int sq_col_b = sqs[b] % grid_x_range;
  
  // compare the columns, rows of the focal and neighbour squares
  int delta_x = sq_col_a - sq_col_b; // focal col - neighbour col
  int delta_y = sq_row_a - sq_row_b; // focal row - neighbour row
  
  // is neighbour next to focal square, or is it on the opposite border?
  int offset_index_x = 0;
  int offset_index_y = 0;
  if (abs(delta_x) > 1)	offset_index_x = delta_x / (int) abs(delta_x);
  if (abs(delta_y) > 1)	offset_index_y = delta_y / (int) abs(delta_y);
...
__kernel void build_matrix_periodic2(const int max_contacts,
									const float muA,
									const float gamma,
									__global const int* sqs,
									__global const float4* centers,
...
__kernel void predict_periodic(__global const float4* centers,
                        __global const float4* dirs,
                        __global const float* lens,
                        __global const float4* dcenters,
                        __global const float4* dangs,
						...
						const float max_y_coord)
{
  ...
  // has the cell gone over a domain edge? 
  // If so, move it back by the appropriate amount
  if (pred_centers[i].x <= min_x_coord){
      pred_centers[i].x += domain_width;
  }
  if (pred_centers[i].x > max_x_coord){
      pred_centers[i].x -= domain_width;  
  }
  if (pred_centers[i].y <= min_y_coord){
      pred_centers[i].y += domain_height;
  }
  if (pred_centers[i].y > max_y_coord){
      pred_centers[i].y -= domain_height;  
  }
}
